...
import (
	...
	"github.com/kolosys/proton/internal/benchmarks"
...
func (g *Generator) Generate() error {
	...
	if g.config.Discovery.Benchmarks.Enabled {
		if err := g.generateBenchmarkDocumentation(packages, context); err != nil {
			return fmt.Errorf("failed to generate benchmark documentation: %w", err)
		}
	}
...
// generateBenchmarkDocumentation generates benchmark documentation
func (g *Generator) generateBenchmarkDocumentation(packages []*discovery.PackageInfo, context *templates.Context) error {
	// Create benchmarks directory
	benchmarksDir := filepath.Join(g.outputPath, "benchmarks")
	if err := os.MkdirAll(benchmarksDir, 0755); err != nil {
		return fmt.Errorf("failed to create benchmarks directory: %w", err)
	}

	// Create benchmark runner
	runner := benchmarks.New(g.projectPath)

	// Determine pattern to use
	pattern := g.config.Discovery.Benchmarks.Pattern
	if pattern == "" {
		pattern = "./..."
	}

	// Run benchmarks or load from file
	var allBenchmarks []*benchmarks.PackageBenchmarks
	var err error

	if g.config.Discovery.Benchmarks.OutputFile != "" {
		// Load from file
		benchmarkFile := g.config.Discovery.Benchmarks.OutputFile
		if !filepath.IsAbs(benchmarkFile) {
			benchmarkFile = filepath.Join(g.projectPath, benchmarkFile)
		}
		allBenchmarks, err = runner.RunFromFile(benchmarkFile)
	} else {
		// Run benchmarks
		allBenchmarks, err = runner.Run(pattern)
	}

	if err != nil {
		return fmt.Errorf("failed to run/load benchmarks: %w", err)
	}

	// Generate benchmark documentation for each package
	for _, pkg := range packages {
		// Skip main packages
		if pkg.Name == "main" {
			continue
		...
		for _, bench := range pkgBenchmarks {
			templateBenchmarks = append(templateBenchmarks, &templates.BenchmarkResult{
				Name:        bench.Name,
		...
		// Generate benchmark documentation
		benchmarkPath := filepath.Join(benchmarksDir, fmt.Sprintf("%s.md", pkg.Name))
		if err := g.templates.RenderToFile("benchmarks", benchmarkContext, benchmarkPath); err != nil {
			return fmt.Errorf("failed to generate benchmark documentation for package %s: %w", pkg.Name, err)
		}
	}
